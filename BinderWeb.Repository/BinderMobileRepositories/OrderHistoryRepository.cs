...
namespace BinderWeb.Repository.BinderWebRepositories
{
    public class OrderHistoryRepository : BinderBaseRepository<OrderHistoryDto>, IOrderHistoryRepository
    {
        ...
        {
            string query = string.Format(@"Select StateId,DealerId,PickupDate,case when ProductOrder.SlotId=1 then ltrim(right(convert(varchar(25),PricingSlote.FromTime, 100), 7))+'-'+ltrim(right(convert(varchar(25),PricingSlote.ToTime, 100), 7)) end as  TimeSlot,UnitPrice,Quantity,TotalPrice
            ,Branch.BRANCHNAME as PickupPoint,ProductName,OrderDate,OrderId,
            OrderNo,'' as StateName,SOC from ProductOrder
            left join PricingSlote on PricingSlote.SlotId=ProductOrder.SlotId
